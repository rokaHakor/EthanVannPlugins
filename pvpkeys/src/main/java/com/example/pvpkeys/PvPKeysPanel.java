...
public class PvPKeysPanel extends PluginPanel
{
	...
	PvPKeysPanel(Client client, pvpkeys plugin) throws IOException
	{
		...
		Files.walk(path).forEach(filePath ->
		{
			if (Files.isRegularFile(filePath))
			{
				if (filePath.getFileName().toString().endsWith(".txt"))
				{
					...
					List<String> itemTriggers = new ArrayList<>();
					try
					{
						...
						for (String line : lines)
						{
                            ...
                            if(line.contains("itemtrigger")){
								if (line.split(":").length == 2)
								{
									itemTriggers.add(line.split(":")[1]);
								}
                            }
						}
						setup.commands = commands.toArray(new String[0]);
						setup.itemTriggers = itemTriggers.toArray(new String[0]);
		...
		textArea.setCaretColor(Color.CYAN.brighter());
		...
		itemArea.getDocument().addDocumentListener(new DocumentListener()
		{
			...
			public void removeUpdate(DocumentEvent e)
			{
                ...
                for (Setup setup1 : plugin.setupList)
                {
                    if (setup1.name.equals(setup))
                    {
                        setup1.itemTriggers = itemArea.getText().split("\n");
                        try
                        {
                            plugin.writeSetupToFile(setup1);
                        }
					...
					if (e.getStateChange() == ItemEvent.SELECTED)
					{
						...
						else
						{
							...
							if (setup != null)
							{
								...
								itemTriggers = itemTriggers.isBlank() ? "" : itemTriggers.trim();
								//							client.getLogger().warn("loading commands: " + commands);
								itemArea.setText(itemTriggers);
		...
		itemArea.setForeground(Color.CYAN);
		itemArea.setCaretColor(Color.CYAN.brighter());
		JPanel helperPanel = new JPanel();
		JLabel helperLabel = new JLabel("Item Triggers");
		helperPanel.setLayout(new BorderLayout());
		helperPanel.add(itemArea, BorderLayout.SOUTH);
		helperPanel.add(helperLabel, BorderLayout.CENTER);
		helperPanel.add(gearButton, BorderLayout.NORTH);
		add(helperPanel);
